...
$string['completedallexcept:activities_help'] = 'Comma-separated list of activities to exclude, identified by course module ID or by ID number. Matches users who have completed every activity with completion tracking in the course apart from the listed ones; the stricter operator additionally requires that none of the listed activities are complete. Only rows of courses containing at least one of the listed activities are shown, so other courses a user has fully completed do not match. An empty list disables the filter. Note that hidden activities and activities with access restrictions still count as trackable, and activities whose completion tracking has been switched off are ignored entirely, including any completion recorded while it was still on.';
...
$string['settings:maxfilteroptions'] = 'Maximum filter options';
$string['settings:maxfilteroptions_desc'] = 'How many options a select filter may collect when its options come '
    . 'from scanning a report\'s rows (optionsfield=...). A dropdown over a large population — every user of the '
    . 'site, say — needs a higher value than the default 500, or its list is silently cut short; a lower value '
    . 'keeps very long dropdowns off the page. Options declared by a report\'s own select filter are never capped.';
